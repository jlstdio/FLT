...
def test_model(fed_model, args, testloader, device, cr):
    ...
    fed_model.to('cpu')
    fed_model.train()
    torch.save(fed_model.state_dict(),
               os.path.join(args.weight_save_path, '%s_%d_%.4f.pth' % (args.comm_type, cr, fed_accuracy)))
...
def start_fedavg(fed_model, args,
    ...
    for cr in range(1, args.comm_round + 1):
        ...
        for edge_progress, edge_index in enumerate(selected_edge):
            ...
            print("[%2d/%2d] edge: %d, data len: %d" % (
            edge_progress, len(selected_edge), edge_index, len(train_data_set)))
...
def start_fedtwa(fed_model, args,
    ...
    for cr in range(1, args.comm_round + 1):
        ...
        for k, edge in enumerate(edges):
            ...
            for key in fed_model.state_dict().keys():
                if k == 0:
                    update_state[key] = local_state[key] * (net_data_count[k] / total_data_count) * math.pow(twa_exp, -(
                                cr - 2 - time_stamp[k]))
                else:
                    update_state[key] += local_state[key] * (net_data_count[k] / total_data_count) * math.pow(twa_exp,
                                                                                                              -(cr - 2 -
...
def start_feddw(fed_model, args,
    ...
    for cr in range(1, args.comm_round + 1):
        ...
        for worker_index in range(args.n_nets):
            candidates.append((H * worker_selected_frequency[worker_index] / sum_frequency + (1 - H) * net_weight_dict[
        ...
        weighted_frequency = [P * (avg_selected_frequency - worker_frequency) for worker_frequency in
                              worker_selected_frequency]
...
def start_only_edge(args,
    ...
    for cr in range(1, total_epoch + 1):
        ...
        torch.save(edges[0].state_dict(),
                   os.path.join(args.weight_save_path, 'edge_%d_%.4f.pth' % (cr, edge_accuracy_avg)))
...
def start_train():
    ...
    print("Loading data...")
    if args.model == 'one':
        train_data_set, data_idx_map, net_data_count, test_data_set = dataset.GetCanDataset(args.n_nets, args.fold_num,
                                                                                            args.packet_num,
        ...
        train_data_set, data_idx_map, net_data_count, test_data_set = dataset.GetCanDatasetCNN(args.n_nets,
                                                                                               args.fold_num,
                                                                                               "./dataset/Mixed_dataset.csv",
                                                                                               "./dataset/Mixed_dataset_CNN8.txt")

...
def pick_clients(self):
        ...
        if self.serverConfig['pickMode'] == 'init_and_random':
            from server.picking_clients.random_pick_clients import init_pick_and_random_pick_clients
            ...
            pickedClients = init_pick_and_random_pick_clients(initial_data, self.rng, 5, 3)
        ...
        elif self.serverConfig['pickMode'] == 'init_cluster_and_mixed_clustered':
            from server.picking_clients.mixed_clustered_pick_clients import mixed_clustered_pick_clients_with_initial_cluster
            ...
            pickedClients, numCluster, type_info_by_clients = mixed_clustered_pick_clients_with_initial_cluster(
                initial_data=initial_data,
                rng=self.rng,
                initial_n_rounds = 5, 
                initial_cluster_idx = 3,
                random_cluster_pick=True,
                rounds_per_cluster_change=self.serverConfig['update_cluster_every'],
                num_clusters_to_pick=num_clusters_to_pick)

        elif self.serverConfig['pickMode'] == 'multi_style_cluster_random':
            from server.picking_clients.clustered_pick_clients import multi_cluster_pick_clients
            ...
            num_clusters_to_pick = self.serverConfig.get('num_clusters_to_pick', 2)
            clients_per_cluster = self.basicConfig['updateClientsPerRound'] // num_clusters_to_pick
            
            pickedClients, numCluster, type_info_by_clients = multi_cluster_pick_clients(
                initial_data=initial_data, 
                rng=self.rng, 
                num_clusters_to_pick=num_clusters_to_pick,
                num_clients_per_cluster=clients_per_cluster,
                cluster_class_distributions=None)

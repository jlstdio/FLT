...
class server_adaptive_guided_fed(server_parent):
    def __init__(self, rootModel, examinDataset_list, serverConfig, basicConfig, currentRound, flipboard,
        ...
        self.guideDataset_list = []

        for examinDataset in examinDataset_list:
            y_server, x_server = zip(*examinDataset)
            len_of_server_dset = len(y_server)
            ds_split = int(len_of_server_dset/2)
            
            self.examinDataset_list.append(zip(y_server[:ds_split], x_server[:ds_split]))
            self.guideDataset_list.append(zip(y_server[ds_split:], x_server[ds_split:]))
    ...
    def run_FL(self):
        ...
        self.flModel = server_type_loader(self, examinDataset_combined, self.guideDataset_list)
        self.flModel.flush()
        ...
        for idx, (dataset_select) in enumerate(self.examinDataset_list):
            ...
            ## TODO
            sub_model_path = f'{rootModelPath}/sub_{idx}_rootModel-{testName}.pth'

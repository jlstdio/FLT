...
class client_2way_fed_distillation(client_parent):
    ...
    def compute_kl_loss(self, student_logits, teacher_logits, temperature):
    ...
    def train(self, epochs=10):
        self.temperature = self.clientProfile['clientMetadata'].get('T', 4.0)  # Distillation temperature
        self.alpha = self.clientProfile['clientMetadata']['penalty_lambda']  # Distillation
        ...
        # Train ##############################
        self.model.train()  # Student mode
        self.prox_model.eval()  # Teacher mode
        ...
        for epoch in range(epochs):
            ...
            for inputs, targets in self.train_loader:
                ...
                # Get student (sub_root) outputs
                student_outputs = self.model(inputs)
                
                with torch.no_grad():
                    # Get teacher (main_root) outputs
                    teacher_outputs = self.prox_model(inputs)

                # Task-specific loss (student predictions vs true targets)
                task_loss = self.criterion(student_outputs, targets)
                
                # Knowledge distillation loss
                distill_loss = self.compute_kl_loss(
                    student_outputs, 
                    teacher_outputs, 
                    self.temperature
                )
                ...
                all_outputs.extend(student_outputs.detach().cpu().numpy())
